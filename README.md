...
> MeetFlow AI 是我设计并实现的一款面向项目协作场景的 AI 会议助手。项目来源于会议纪要整理中的真实痛点：会后整理耗时长、TODO 容易遗漏、责任人和截止时间不清晰、会议结论难以追溯。
>
> V1 版本聚焦会议后处理链路，支持文本输入和音频上传两种方式。音频输入会先通过 faster-whisper 转写为 transcript，再由大模型分阶段提取会议概览、摘要、关键决策、TODO 和风险问题。
>
> 为了提升大模型输出的稳定性，我没有直接信任模型结果，而是在后端加入了 Pydantic 校验层，对字段类型、枚举值和必填字段进行约束。同时保留 transcript_text、evidence_time 等字段，支持结果追溯。
>
> 在产品边界上，V1 没有做实时录音、飞书同步和复杂任务系统，而是先验证“会议内容 → 结构化纪要 → 可导出结果”的核心闭环。后续可以扩展前端编辑、任务状态追踪、飞书多维表格同步和历史会议 RAG 查询。